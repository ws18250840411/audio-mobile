for (var d, f, l = document.getElementsByTagName("head")[0].style, h = ["transformProperty", "WebkitTransform","OTransform", "msTransform", "MozTransform"], g = 0; g < h.length; g++) void 0 !== l[h[g]] && (d = h[g]);d && (f = d.replace(/[tT]ransform/, "TransformOrigin"), "T" == f[0] && (f[0] = "t"));
function isIE(){
...
QKrotate.prototype = {
    ...
    createElement: function(options){
        ...
        document.querySelector('.audios').appendChild(parentNodes);     
    },
    /**
     * 生成radio节点
     * @private
    ...
    setEditorEvents:function(){
        var that = this;
        var audioCon = document.querySelector('.audios'),
        ...
        if (audioCore.play || (audioCore.play = audioCore.pause = function() {}),audioCore.onplay = changeSty,audioCore.onended = audioCore.onpause = changeSty,changeSty(), !{}.inEditor) {
            ...
            audioCon.addEventListener("click", function() {
    ...
    rotate:function(options){
        var that = this;
        var ele = document.querySelector('.audios');
        that.timer = null;
        if(that.options.isRotate){
            ...
            that.timer = setInterval(function() {  
                angle += 10; 
                that.startRotate(ele,angle);
            }, that.options.speed)  
        }
         
    },
    startRotate:function(ele,a){
        //旋转
        var el = this.getElementByClassName('audios');
        if (0 !== this.length && "undefined" != typeof a) {
            ...
            for (var b = [], c = 0, d = el.length; c < d; c++) {
                var e = ele;
                if (e.QKrotate && e.QKrotate.PhotoEffect){
                    e.QKrotate.PhotoEffect._handleRotation(a);
                } else {
                    var f = this.extend(!0, {}, a),
                        e = (new QKrotate.PhotoEffect(e, f))._rootObj; 
                    b.push(e)
                }
            }
            return b
    ...
    stopRotate:function(){
        var that = this.getElementByClassName('audios');
        if(this.timer){
            clearTimeout(this.timer);
            this.timer = null;
        }
        for (var a = 0, b = that.length; a < b; a++) {
            var c = document.querySelector('.audios');
            c.QKrotate && c.QKrotate.PhotoEffect && clearTimeout(c.QKrotate.PhotoEffect._timer)
        }
    },
...
QKrotate.PhotoEffect = function () {
    ...
    return d ? function (a, b) {
        ...
        if (a.complete) this._Loader();
            ...
            var c = this;
            var target = this._img;
            if (target.addEventListener) {// 非ie 和ie9
                target.addEventListener("load", function(){c._Loader()}, false);
            } else if (target.attachEvent) { // ie6到ie8
                target.attachEvent("onload",  function(){c._Loader()});
            } else {
                target["onload"] =  function(){c._Loader()}; // ie5
            }
        }

    }
}();
QKrotate.PhotoEffect.prototype = {
        ...
        if (a && this._eventObj) {
            if (this._parameters.bind) {
                var b = this._parameters.bind,
                    c;
                for (c in b) b.hasOwnProperty(c) && this._unbind(c, b[c])
            }
            this._parameters.bind = a;
            for (c in a) a.hasOwnProperty(c) && this._unbind(this._eventObj,c, a[c])
        }
    },
    _bind:function (target, type, func) {
        if (target.addEventListener) {// 非ie 和ie9
    ...
    _unbind: function (target, type, func) {
    ...
    _Loader: function () {
        return isIE() ? function () {
            ...
            this._canvas.QKrotate = this._rootObj.QKrotate;
...
isIE() && (QKrotate.PhotoEffect.prototype.createVMLNode = function () {
